...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated GluonCV loader file *)


Unprotect[GluonCV`PackageScope`Private`$TopLevelLoad];
GluonCV`PackageScope`Private`$TopLevelLoad=MemberQ[$ContextPath, "Global`"];


BeginPackage["GluonCV`"];


ClearAll[GluonCV];
GluonCV::usage="GluonCV is a manager head for the GluonCV package";
...
GluonCV["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
GluonCV["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
GluonCV["Contexts"]:=$PackageContexts;
If[!ListQ@$PackageContexts,
  $PackageContexts=
    {
      "GluonCV`",
      "GluonCV`PackageScope`Private`",
      "GluonCV`PackageScope`Package`"
      }
  ];
$PackageDeclared=
...
GluonCV["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
GluonCV["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
GluonCV["FileContexts"]:=$PackageFileContexts;
...
GluonCV["DeclaredPackages"]:=$DeclaredPackages;
...
GluonCV["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,GluonCV],"`"]
    ];
...
GluonCV::nodep="Couldn't load dependency `` of type ``";
GluonCV::nodup="Couldn't update dependency `` of type ``";
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[GluonCV::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[GluonCV::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[GluonCV::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[GluonCV, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageFailureException[
  ...
  (
    Set[MessageName[GluonCV, tag], body];
    PackageFailureException[
      MessageName[GluonCV, tag],
      body,
      ops
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[GluonCV, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[GluonCV],
...
If[
  (Clear@GluonCV`PackageScope`Private`$loadAbort;!#)&@
    GluonCV`PackageScope`Private`$loadAbort,
  Unprotect[GluonCV`PackageScope`Private`$LoadCompleted];
  GluonCV`PackageScope`Private`$LoadCompleted=True;
  GluonCV`PackageScope`Private`PackagePostProcessContextPathReassign[]
  ]