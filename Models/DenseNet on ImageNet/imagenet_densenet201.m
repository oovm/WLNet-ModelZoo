...
ndarry = NDArrayImport["imagenet_densenet201-0000.params"];
...
getBN[i_, j_] := BatchNormalizationLayer[
	"Epsilon" -> 1*^-5,
	"Beta" -> ndarry["arg:densenet3_stage" <> i <> "_batchnorm" <> j <> "_beta"],
	"Gamma" -> ndarry["arg:densenet3_stage" <> i <> "_batchnorm" <> j <> "_gamma"],
	"MovingMean" -> ndarry["aux:densenet3_stage" <> i <> "_batchnorm" <> j <> "_running_mean"],
	"MovingVariance" -> ndarry["aux:densenet3_stage" <> i <> "_batchnorm" <> j <> "_running_var"]
]
getBN2[j_] := BatchNormalizationLayer[
	"Epsilon" -> 1*^-5,
	"Beta" -> ndarry["arg:densenet3_batchnorm" <> j <> "_beta"],
	"Gamma" -> ndarry["arg:densenet3_batchnorm" <> j <> "_gamma"],
	"MovingMean" -> ndarry["aux:densenet3_batchnorm" <> j <> "_running_mean"],
	"MovingVariance" -> ndarry["aux:densenet3_batchnorm" <> j <> "_running_var"]
]
getCN[i_, j_, p_ : 1, s_ : 1] := ConvolutionLayer[
	"Weights" -> ndarry["arg:densenet3_stage" <> i <> "_conv" <> j <> "_weight"],
	"Biases" -> None, "PaddingSize" -> p, "Stride" -> s
]
getCN2[j_, p_ : 1, s_ : 1] := ConvolutionLayer[
	"Weights" -> ndarry["arg:densenet3_conv" <> j <> "_weight"],
	"Biases" -> None, "PaddingSize" -> p, "Stride" -> s
]
$getBlock = NetChain@{
...
$getBlock2 = NetChain@{
	...
	LinearLayer[1000,
		"Weights" -> ndarry["arg:densenet3_dense0_weight"],
		"Biases" -> ndarry["arg:densenet3_dense0_bias"]
	]
}
...
Export["imagenet_densenet201.WMLF", mainNet]