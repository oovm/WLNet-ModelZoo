...
params = NDArrayImport["imagenet_vgg19_bn-0000.params"];
...
getCN[i_] := ConvolutionLayer[
	"Weights" -> params["arg:vgg3_conv" <> i <> "_weight"],
	"Biases" -> params["arg:vgg3_conv" <> i <> "_bias"],
	"PaddingSize" -> 1, "Stride" -> 1
];
getBN[i_] := BatchNormalizationLayer[
	"Epsilon" -> 1*^-5,
	"Beta" -> params["arg:vgg3_batchnorm" <> i <> "_beta"],
	"Gamma" -> params["arg:vgg3_batchnorm" <> i <> "_gamma"],
	"MovingMean" -> params["aux:vgg3_batchnorm" <> i <> "_running_mean"],
	"MovingVariance" -> params["aux:vgg3_batchnorm" <> i <> "_running_var"]
]
getFC[i_, n_] := LinearLayer[n,
	"Weights" -> params["arg:vgg3_dense" <> i <> "_weight"],
	"Biases" -> params["arg:vgg3_dense" <> i <> "_bias"]
];
...
Export["imagenet_vgg19_bn.WMLF", mainNet]
