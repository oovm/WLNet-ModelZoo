...
params = NDArrayImport["imagenet_vgg11_bn-0000.params"];
...
getBN[i_] := BatchNormalizationLayer[
	"Epsilon" -> 1*^-5,
	"Beta" -> params["arg:vgg0_batchnorm" <> i <> "_beta"],
	"Gamma" -> params["arg:vgg0_batchnorm" <> i <> "_gamma"],
	"MovingMean" -> params["aux:vgg0_batchnorm" <> i <> "_running_mean"],
	"MovingVariance" -> params["aux:vgg0_batchnorm" <> i <> "_running_var"]
]
...
Export["imagenet_vgg11_bn.WMLF", mainNet]
