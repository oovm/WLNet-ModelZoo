...
params = NDArrayImport["imagenet_vgg13_bn-0000.params"];
...
getBN[i_] := BatchNormalizationLayer[
	"Epsilon" -> 1*^-5,
	"Beta" -> params["arg:vgg1_batchnorm" <> i <> "_beta"],
	"Gamma" -> params["arg:vgg1_batchnorm" <> i <> "_gamma"],
	"MovingMean" -> params["aux:vgg1_batchnorm" <> i <> "_running_mean"],
	"MovingVariance" -> params["aux:vgg1_batchnorm" <> i <> "_running_var"]
]
...
Export["imagenet_vgg13_bn.WMLF", mainNet]
