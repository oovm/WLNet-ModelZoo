...
params = NDArrayImport["imagenet_densenet161-0000.params"];
...
getBN[i_, j_] := BatchNormalizationLayer[
	"Epsilon" -> 1*^-5,
	"Beta" -> params["arg:densenet1_stage" <> i <> "_batchnorm" <> j <> "_beta"],
	"Gamma" -> params["arg:densenet1_stage" <> i <> "_batchnorm" <> j <> "_gamma"],
	"MovingMean" -> params["aux:densenet1_stage" <> i <> "_batchnorm" <> j <> "_running_mean"],
	"MovingVariance" -> params["aux:densenet1_stage" <> i <> "_batchnorm" <> j <> "_running_var"]
];
getBN2[j_] := BatchNormalizationLayer[
	"Epsilon" -> 1*^-5,
	"Beta" -> params["arg:densenet1_batchnorm" <> j <> "_beta"],
	"Gamma" -> params["arg:densenet1_batchnorm" <> j <> "_gamma"],
	"MovingMean" -> params["aux:densenet1_batchnorm" <> j <> "_running_mean"],
	"MovingVariance" -> params["aux:densenet1_batchnorm" <> j <> "_running_var"]
];
getCN[i_, j_, p_ : 1, s_ : 1] := ConvolutionLayer[
	"Weights" -> params["arg:densenet1_stage" <> i <> "_conv" <> j <> "_weight"],
	"Biases" -> None, "PaddingSize" -> p, "Stride" -> s
];
getCN2[j_, p_ : 1, s_ : 1] := ConvolutionLayer[
	"Weights" -> params["arg:densenet1_conv" <> j <> "_weight"],
	"Biases" -> None, "PaddingSize" -> p, "Stride" -> s
];
...
extractor = NetChain[{
	...
	getBlock2[2],
	NetChain@Table[getBlock[3, i], {i, 0, 70, 2}],
	getBlock2[3],
	NetChain@Table[getBlock[4, i], {i, 0, 46, 2}],
	$getBlock2
}];
classifier = LinearLayer[1000,
	"Weights" -> params["arg:densenet1_dense0_weight"],
	"Biases" -> params["arg:densenet1_dense0_bias"]
];
mainNet = NetChain[{
	"Extractor" -> extractor,
...
Export["DenseNet161 trained on ImageNet.WXF", mainNet]
