...
mainNet = NetGraph[{
	"_Mask" -> {FlattenLayer[1], ReplicateLayer[3]},
	"^Mask" -> {AggregationLayer[Max, 1], ReplicateLayer[1]},
	"Enc_1" -> getCN[49, 3, 2],
	"Act_1" -> ElementwiseLayer["ReLU"],
	"Enc_2" -> getCN[50, 2, 2],
	"Act_2" -> {getBN[1], Ramp},
	...
	"Enc_8" -> getCN[56, 1, 2],
	"Act_8" -> {getBN[7], Ramp},
	"Iup_1" -> UpSampleMask, (*Looks Like So*)
	"Mup_1" -> UpSampleMask,
	"Dec_1" -> getCN[57, 1, 1],
	"Iup_2" -> UpSampleImage[22],
	"Mup_2" -> UpSampleMask,
	"Dec_2" -> getCN[58, 1, 1],
	"Iup_3" -> UpSampleImage[23],
	"Mup_3" -> UpSampleMask,
	"Dec_3" -> getCN[59, 1, 1],
	"Iup_4" -> UpSampleImage[24],
	"Mup_4" -> UpSampleMask,
	"Dec_4" -> getCN[60, 1, 1],
	"Iup_5" -> UpSampleImage[25],
	"Mup_5" -> UpSampleMask,
	"Dec_5" -> getCN[61, 1, 1],
	"Iup_6" -> UpSampleImage[26],
	"Mup_6" -> UpSampleMask,
	"Dec_6" -> getCN[62, 1, 1],
	"Iup_7" -> UpSampleImage[27],
	"Mup_7" -> UpSampleMask,
	"Dec_7" -> getCN[63, 1, 1],
	"Iup_8" -> UpSampleImage[28],
	"Mup_8" -> UpSampleMask,
	"Dec_8" -> getCN[64, 1, 1],
	"Act^8" -> {
		ElementwiseLayer[Ramp[#] - 0.2 * Ramp[-#]&],
	...
	Flatten@{
		NetPort["InputMask"] -> "_Mask",
		"^Mask" -> NetPort["OutputMask"],
	(*UNet-Encoders*)
		{NetPort["Input"], "_Mask"} -> "Enc_1",
		NetPort["Enc_1", "Output"] -> "Act_1",
		{"Act_1", NetPort["Enc_1", "OutputMask"]} -> "Enc_2",
		NetPort["Enc_2", "Output"] -> "Act_2",
		{"Act_2", NetPort["Enc_2", "OutputMask"]} -> "Enc_3",
	...
	(*UNet-Decoders*)
		...
		{"Iup_1", "Mup_1"} -> "Dec_1",
	(*
	{"Act_6",NetPort["Dec_1","Output"]}->"Iup_2",
	{NetPort["Enc_6","OutputMask"],NetPort["Dec_1","OutputMask"]}->"Mup_2",
	{"Iup_2","Mup_2"}->"Dec_2",
	*)
		Table[{
			{"Act_" <> ToString[7 - i], NetPort["Dec_" <> ToString[i], "Output"]} -> "Iup_" <> ToString[i + 1],
			{NetPort["Enc_" <> ToString[7 - i], "OutputMask"], NetPort["Dec_" <> ToString[i], "OutputMask"]} -> "Mup_" <> ToString[i + 1],
			{"Iup_" <> ToString[i + 1], "Mup_" <> ToString[i + 1]} -> "Dec_" <> ToString[i + 1]
		}, {i, 6}],
		
		{NetPort["Input"], NetPort["Dec_7", "Output"]} -> "Iup_8",
		{"_Mask", NetPort["Dec_7", "OutputMask"]} -> "Mup_8",
		{"Iup_8", "Mup_8"} -> "Dec_8",
	(*Output*)
		NetPort["Dec_8", "OutputMask"] -> "^Mask",
		NetPort["Dec_8", "Output"] -> "Act^8" -> NetPort["Output"]
	},
	"Input" -> NetEncoder[{"Image", 256, "VarianceImage" -> 1}],
	"InputMask" -> NetEncoder[{"Image", 256, ColorSpace -> "Grayscale"}],
	"Output" -> NetDecoder["Image"],
	"OutputMask" -> NetDecoder["Image"]
]